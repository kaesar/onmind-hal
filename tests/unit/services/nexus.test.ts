...
import { NexusService } from '../../../src/services/optional/nexus.js';
...
describe('NexusService', () => {
  ...
  beforeEach(() => {
    config = {
      ...
      selectedServices: [ServiceType.NEXUS],
      distribution: DistributionType.UBUNTU
    };

    const templateLoader = new TemplateLoader();
    templateEngine = new TemplateEngine(templateLoader);
    service = new NexusService(config, templateEngine);
  });

  it('should have correct service name', () => {
    expect(service.name).toBe('Nexus Repository');
  });

  it('should have correct service type', () => {
    expect(service.type).toBe(ServiceType.NEXUS);
  });
