...
import { FluentBitService } from '../../../src/services/optional/fluentbit.js';
...
describe('FluentBitService', () => {
  let service: FluentBitService;
  ...
  beforeEach(() => {
    config = {
      ...
      selectedServices: [ServiceType.FLUENTBIT],
      distribution: DistributionType.UBUNTU
    };

    const templateLoader = new TemplateLoader();
    templateEngine = new TemplateEngine(templateLoader);
    service = new FluentBitService(config, templateEngine);
  });

  it('should have correct service name', () => {
    expect(service.name).toBe('Fluent Bit');
  });

  it('should have correct service type', () => {
    expect(service.type).toBe(ServiceType.FLUENTBIT);
  });
  ...
  it('should return correct access URL', () => {
    ...
    expect(url).toBe('http://192.168.1.100:2020');
  });
});
